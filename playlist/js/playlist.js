...
const musicCatalog = () => {
    ...
    const addSongToPlaylist = (playlistName, song) => {
      ...
      playlists = playlists.map(pl => {
        if (pl.name === playlistName) {
          return {...pl, songs: [...pl.songs, {...song, favorite: song.favorite ?? false}]};
        } else {
          return pl;
        }
      })
    ...
    const removeSongFromPlaylist = (playlistName, title) => {
     ...
     playlists = playlists.map(pl => {
        if (pl.name === playlistName) {
          return { ...pl, songs: pl.songs.filter(song => song.title !== title)};
         } else {
          return pl;
         }
    });
    ...
    const favoriteSong = (playlistName, title) => {
      ...
      const song = playlist.songs.find(sng => sng.title === title);
      if (!song) {
        throw new Error(`La canción ${title} no existe en esta playlist`)
      }
